...
def create_assistant():
    global assistant_id
    my_assistant = client.beta.assistants.retrieve(assistant_id="asst_IGem2dehhudk94s9ffveO4iR")
...
def chat():
    ...
    # send and receive messages from assistant API
    ...
    # displays animated dots when loading
    while run.status != "completed":
        time.sleep(0.5)
        run = client.beta.threads.runs.retrieve(thread_id = thread_id, run_id = run.id)

    # message from assistant is added to chat history
    ...
    # run log function and return message
    log_run(run.status)
